...
class DatabaseManager:
    ...
    def create_session(self, filenames):
        ...
        cur = self.conn.cursor()
        cur.execute(
            "INSERT INTO sessions (session_name, timestamp) VALUES (?, ?)",
            (name, datetime.now()),
        )
        self.conn.commit()
    ...
    def add_query_record(self, session_id, question, response, sources):
        self.conn.execute(
            "INSERT INTO queries (session_id, question, response, sources_json, timestamp) VALUES (?, ?, ?, ?, ?)",
    ...
    def get_all_sessions(self):
        return self.conn.execute(
            "SELECT s.id, s.session_name, s.timestamp, COUNT(d.id) FROM sessions s LEFT JOIN documents d ON s.id=d.session_id GROUP BY s.id ORDER BY s.timestamp DESC"
        ).fetchall()
    ...
    def get_queries_for_session(self, session_id):
        cur = self.conn.execute(
            "SELECT question, response, sources_json FROM queries WHERE session_id=? ORDER BY timestamp ASC",
            (session_id,),
        )
        return [
