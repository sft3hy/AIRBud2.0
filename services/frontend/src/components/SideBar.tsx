...
const { data: collections = [] } = useQuery({ queryKey: ['collections'], queryFn: getCollections, enabled: !!systemStatus?.online });
const { data: userGroups = [] } = useQuery({ queryKey: ['my_groups'], queryFn: getMyGroups, enabled: !!systemStatus?.online });
const { data: currentDocs = [] } = useQuery({ queryKey: ['documents', currentSessionId], queryFn: () => fetchCollectionDocuments(currentSessionId!), enabled: !!currentSessionId });
const { data: jobStatus } = useQuery({ queryKey: ['status', activeJobId], queryFn: () => getCollectionStatus(activeJobId!), enabled: !!activeJobId, refetchInterval: 1000 });
...
const handleRenameCollection = async () => { if (!renameCid || !renameName.trim()) return; await renameCollection(renameCid, renameName); queryClient.invalidateQueries({queryKey:['collections']}); setRenameCid(null); };

const handleCreateCollection = async () => { 
    if (!newCollectionName.trim()) return; 
    const id = await createCollection(newCollectionName, selectedGroupId === "personal" ? undefined : parseInt(selectedGroupId)); 
...
const handleCreateGroup = async () => { await createGroup(newGroupName, newGroupDesc, newGroupPublic); queryClient.invalidateQueries({queryKey:['my_groups']}); setNewGroupName(""); };
const handleRenameGroup = async () => { if (!renameGid) return; await renameGroup(renameGid, renameGroupName); queryClient.invalidateQueries({queryKey:['my_groups']}); setRenameGid(null); };
const handleDeleteGroup = async (gid: number) => { await deleteGroup(gid); queryClient.invalidateQueries({queryKey:['my_groups']}); };
const handleFileSelect = (e: any) => { if(e.target.files) setStagedFiles(Array.from(e.target.files)); };
const handleAddFiles = async () => { setIsUploading(true); for(const f of stagedFiles) await uploadAndProcessDocument(currentSessionId!, f, selectedModel); setActiveJobId(currentSessionId); setIsUploading(false); setStagedFiles([]); };
const handleDeleteDoc = async (id: number) => { await deleteDocument(id); queryClient.invalidateQueries({queryKey:['documents']}); };
...
// --- RENDER ---
return (
    <div className={cn("flex flex-col h-full w-full bg-muted/10 border-r transition-all duration-300", className)}>
        
        {/* 1. HEADER (Title + Toggle) */}
        <div className={cn("border-b bg-background flex shrink-0 transition-all", isCollapsed ? "p-2 justify-center flex-col gap-2" : "p-4 flex-col gap-4")}>
            
            <div className={cn("flex items-center", isCollapsed ? "justify-center" : "justify-between")}>
                {!isCollapsed && (
                    <h2 className="text-lg font-bold flex items-center gap-2 truncate">
                        🧠 AIRBud 2.0
                        <div className={cn("h-2.5 w-2.5 rounded-full shrink-0", systemStatus?.online ? 'bg-green-500' : 'bg-red-500 animate-pulse')} />
                    </h2>
                )}
                    ...
                    <Tooltip delayDuration={0}>
                        <TooltipTrigger asChild>
                            <Button 
                                ...
                                className={cn("h-8 w-8 text-muted-foreground", isCollapsed && "h-10 w-10")}
                            >
            ...
            {isCollapsed && (
                <div className="flex flex-col gap-2 items-center mt-2">
                     <TooltipProvider>
                        <Tooltip delayDuration={0}>
                            <TooltipTrigger asChild>
                                <Button variant={mode === 'collections' ? "secondary" : "ghost"} size="icon" onClick={() => navigate('/collections')}>
                        ...
                        <Tooltip delayDuration={0}>
                            <TooltipTrigger asChild>
                                <Button variant={mode === 'groups' ? "secondary" : "ghost"} size="icon" onClick={() => navigate('/groups')}>
                            ...
                            {!currentSessionId ? (
                                <ScrollArea className="flex-1 px-4 py-4">
                                    ...
                                    <div className="space-y-4">
                                        <h3 className="text-xs font-bold uppercase text-muted-foreground tracking-wider">Available Collections</h3>
                                                ...
                                                filteredCollections.map(c => {
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-3 overflow-hidden">
                                                            ...
                                                            <div className="min-w-0">
                                                                ...
                                                                <div className="text-[0.8rem] text-muted-foreground truncate">
                                                                    ...
                                                                    {c.group_name && <span> • Group: {c.group_name}</span>}
                                                                </div>
                                                        ...
                                                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
                                                            {isOwner && (
                                                                <>
                                                                    <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-primary" onClick={(e) => { e.stopPropagation(); setRenameCid(c.id); setRenameName(c.name); }}>
                                ...
                                <div className="flex flex-col h-full bg-background">
                                    <div className="p-3 border-b flex items-center gap-2 bg-muted/20">
                                        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => navigate('/collections')}>
                                    ...
                                    <Tabs defaultValue="docs" className="flex-1 flex flex-col min-h-0">
                                                        ...
                                                        <div key={doc.id} className="flex items-center justify-between p-2 bg-card border rounded-md text-xs group">
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <AlertDialog>
                                                                    ...
                                                                    <AlertDialogContent>
                                                                        <AlertDialogHeader><AlertDialogTitle>Delete Document?</AlertDialogTitle><AlertDialogDescription>Remove <b>{doc.original_filename}</b>?</AlertDialogDescription></AlertDialogHeader>
                                                                        <AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={() => handleDeleteDoc(doc.id)} className="bg-destructive">Delete</AlertDialogAction></AlertDialogFooter>
                                                                    </AlertDialogContent>
                                                                </AlertDialog>
                                                            </div>
                                                        </div>
                                                    ...
                                                    <div className="space-y-3 pt-4 border-t">
                                                        <div className="flex justify-between items-center">
                                                            ...
                                                            <Select value={selectedModel} onValueChange={(v) => setSelectedModel(v as VisionModel)}>
                                                                <SelectTrigger className="h-6 text-[0.8rem] w-[10rem]"><SelectValue placeholder="Select a Vision Model"/></SelectTrigger>
                                                                <SelectContent>
                                                                    <SelectGroup>
                                                        ...
                                                        {stagedFiles.length > 0 && (
                                                            <div className="space-y-2">
                                                                {stagedFiles.map((f, i) => (<div key={i} className="flex justify-between text-xs px-1"><span className="truncate">{f.name}</span><X className="h-3 w-3 cursor-pointer" onClick={() => setStagedFiles(p => p.filter(x => x !== f))} /></div>))}
                                                                <Button size="sm" className="w-full h-7 text-xs" onClick={handleAddFiles} disabled={isUploading}>{isUploading ? <Loader2 className="h-3 w-3 animate-spin mr-2" /> : "Process"}</Button>
                                                            </div>
                                                        )}
                                                    </div>
                                                )}
                                        ...
                                        <TabsContent value="charts" className="flex-1 mt-0 overflow-hidden"><div className="h-full p-2"><ChartBrowser collectionId={currentSessionId} /></div></TabsContent>
                                        <TabsContent value="graph" className="flex-1 mt-0 overflow-hidden bg-white"><div className="h-full w-full"><GraphExplorer collectionId={currentSessionId} /></div></TabsContent>
                                    </Tabs>
                                </div>
                            )}
                        </>
        ...
        {/* Rename Dialogs */}
        <Dialog open={!!renameCid} onOpenChange={(open) => !open && setRenameCid(null)}><DialogContent><DialogHeader><DialogTitle>Rename Collection</DialogTitle></DialogHeader><div className="py-4"><Input value={renameName} onChange={(e) => setRenameName(e.target.value)} /></div><DialogFooter><Button variant="outline" onClick={() => setRenameCid(null)}>Cancel</Button><Button onClick={handleRenameCollection} disabled={!renameName.trim()}>Save</Button></DialogFooter></DialogContent></Dialog>
        <Dialog open={!!renameGid} onOpenChange={(open) => !open && setRenameGid(null)}><DialogContent><DialogHeader><DialogTitle>Rename Group</DialogTitle></DialogHeader><div className="py-4"><Input value={renameGroupName} onChange={(e) => setRenameGroupName(e.target.value)} /></div><DialogFooter><Button variant="outline" onClick={() => setRenameGid(null)}>Cancel</Button><Button onClick={handleRenameGroup} disabled={!renameGroupName.trim()}>Save</Button></DialogFooter></DialogContent></Dialog>
    </div>
);
