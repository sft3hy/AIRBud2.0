...
import { FileText, Eye, BrainCircuit, Network, CheckCircle2, ScanLine, Database } from 'lucide-react';
    ...
    <div className="relative w-48 h-48">
        ...
        {[0, 120, 240].map((deg, i) => (
            <motion.div
                ...
                animate={{
                    x: [Math.cos(deg * Math.PI/180) * 60, Math.cos((deg+360) * Math.PI/180) * 60],
                    y: [Math.sin(deg * Math.PI/180) * 60, Math.sin((deg+360) * Math.PI/180) * 60],
                }}
                transition={{ repeat: Infinity, duration: 3, ease: "linear" }}
            />
        ))}
...
const PipelineStep = ({ label, active, completed, icon: Icon }: any) => (
    <div className={`flex flex-col items-center gap-2 transition-all duration-500 ${active ? 'scale-110 opacity-100' : 'opacity-50'} ${completed ? 'text-primary opacity-80' : ''}`}>
        <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-colors ${
            active ? 'border-primary bg-primary/20 text-primary shadow-[0_0_20px_rgba(59,130,246,0.5)]' : 
            completed ? 'border-primary bg-primary text-primary-foreground' : 'border-muted text-muted-foreground'
        }`}>
...
export const ProcessingView: React.FC<ProcessingViewProps> = ({ status }) => {
    ...
    const renderAnim = () => {
        switch(stage) {
            case 'parsing': return <ParsingAnim />;
            case 'vision': return <VisionAnim />;
            case 'indexing': return <IndexingAnim />;
            case 'graph': return <GraphAnim />;
            default: return <BrainCircuit className="w-20 h-20 text-muted-foreground animate-pulse" />;
        }
    };
            ...
            <div className="relative z-10 flex flex-col items-center w-full max-w-2xl">
                ...
                <div className="text-center mb-12 space-y-2">
                    <h2 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-primary to-purple-600 bg-clip-text text-transparent animate-gradient">
                        {stage === 'parsing' ? 'Deconstructing Document' : 
                         stage === 'vision' ? 'Visual Intelligence' :
                         stage === 'indexing' ? 'Vectorization' :
                         stage === 'graph' ? 'Knowledge Mapping' : 'Processing'}
                    </h2>
                ...
                <div className="w-full flex justify-between items-center relative mb-8">
                    ...
                    <PipelineStep label="Layout" icon={FileText} active={stage === 'parsing'} completed={currentIdx > 0} />
                    <PipelineStep label="Vision" icon={Eye} active={stage === 'vision'} completed={currentIdx > 1} />
                    <PipelineStep label="Vectors" icon={Database} active={stage === 'indexing'} completed={currentIdx > 2} />
                    <PipelineStep label="Graph" icon={Network} active={stage === 'graph'} completed={currentIdx > 3} />
                </div>
